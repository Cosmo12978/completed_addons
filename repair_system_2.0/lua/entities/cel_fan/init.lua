...
self:SetModel( "models/uc/props_interior/ceiling_fan.mdl" )
...
if(self.hitsneeded >= 99) then return; end 
	...
	if(self.hitsneeded >= 99) then 
		self.repair = true
			until_restore_cel_fan = CurTime() + 20
			DarkRP.notify(player, 3, 6, "You repaired a ceiling fan! You got Payed $"..moneyaddedforrepair)
...
self:SetNWBool( "repair_state_cel_fan", false )
self:SetNWString('repair_precent_cel_fan',precents.. "% Repaired...")
end

if self.repair == true then
self:SetNWBool( "repair_state_cel_fan", true )
end

if self.repair == true then
if CurTime() < until_restore_cel_fan then return end
self.hitsneeded = math.random(  0,15  )
self.repair = false
self:SetNWBool( "repair_state_cel_fan", false )
end
