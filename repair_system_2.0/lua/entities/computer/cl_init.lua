...
if (self:GetPos():DistToSqr(lp:GetPos()) > 300*300) then return end
 if RPExtraTeams[lp:Team()].name ~= "Medic" then return end

	local Pos = self:GetPos()
	local Ang = Angle( 0, 0, 12)
if self:GetNWBool( "repair_state_computer") == false then

	Ang:RotateAroundAxis(Ang:Forward(), 90)
	local TextAng = Ang
	TextAng:RotateAroundAxis(TextAng:Right(), -90)
	cam.Start3D2D(Pos + Ang:Right() * -40, Angle(0, LocalPlayer():EyeAngles().y-90, 90), 0.1)
		draw.WordBox(.9, -50, 0, self:GetNWString('repair_precent_computer'), "Repair_font", Color(0, 0, 0, 155), Color(255,255,255,255))
	cam.End3D2D()
end
end
